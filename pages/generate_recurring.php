...
try {
    // Fetch all active recurring transactions scheduled for today
    $stmt = $pdo->prepare("SELECT rt.*
        FROM recurring_transactions rt
        WHERE rt.is_active = 1
          AND rt.day_of_month = ?
          AND (rt.last_generated IS NULL
    ...
    foreach ($recurrings as $rec) {
        try {
            // Insert into expenses or incomes
            if ($rec['type'] === 'expense') {
                $insert = $pdo->prepare("INSERT INTO expenses
                        (user_id, card_id, category, montant, decription, dates)
                        VALUES(?, ?, ?, ?, ?, ?)");
            } else { // income
                $insert = $pdo->prepare("INSERT INTO incomes
                        (user_id, card_id, category, montant, decription, dates)
                         VALUES(?, ?, ?, ?, ?, ?)");
            }
            ...
            $update = $pdo->prepare("UPDATE recurring_transactions
                SET last_generated = ?
                WHERE id = ?");
