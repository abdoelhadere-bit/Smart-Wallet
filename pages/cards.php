...
foreach($cards as $i => $card) {
    ...
    // transferts sortants (sender)
    $sqlOut = $pdo->prepare("SELECT SUM(amount) total FROM transfers WHERE sender_id = ? AND card_id = ?");
    $sqlOut->execute([$user_id, $card['id']]);
    $out = $sqlOut->fetch()['total'] ?? 0;
    
    // transferts entrants (receiver)
    $sqlIn = $pdo->prepare("SELECT SUM(amount) total FROM transfers WHERE receiver_id = ? AND card_id = ?");
    $sqlIn->execute([$user_id, $card['id']]);
    $in = $sqlIn->fetch()['total'] ?? 0;
    
    $cards[$i]['current_balance'] = $card['initial_balance'] + $incomes - $expenses - $out + $in;
